...
On a Bash command line, you can flip a `DUSK_CHROME` environment variable boolean flag:

```
php artisan dusk # runs in Firefox
export DUSK_CHROME=1
php artisan dusk # runs in Google Chrome
unset DUSK_CHROME
php artisan dusk # again runs in Firefox
```

> The `tests/DuskTestCase.php` generated by `--with-chrome` assumes you are not running command `php artisan cache:config` before `php artisan dusk`. If this is not the case, you will need to replace `env('DUSK_CHROME')` with a `config()` key stored in your application.

<a name="selecting-browser-in-ci"></a>
#### Selecting desired browser in continuous integration

If you run automated test flows through tools such as Chipper CI, CircleCI, Travis CI, or Github Actions, you can setup one job to run Google Chrome and a second job for Firefox. Similar to running tests locally, make one job set environment variable `DUSK_CHROME=1` before running `php artisan dusk`. With the environment variable undefined in the second job's configuration, `php artisan dusk` will default to Firefox.
